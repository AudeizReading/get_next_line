...
/*   Updated: 2021/05/28 11:50:47 by alellouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	static char		*buf;*/
	...
	while (ret > 0 && !ft_strchr(buf, '\n'))
	{
		if (!ft_strchr(buf, '\n'))
		{
			...
			buf[ret] = 0;
		/*	printf("\033[1;34mBorn to be while\033[0m\n");
			printf("\033[1;35mret read : %d\033[0m\n", ret);
			printf("\033[1;36mbuf : %s\033[0m\n", buf);
			printf("\033[1;37mnewline : %s\033[0m\n", newline);
			printf("The end of the while\n");*/
			octets_lus += ret;
		}
	}
	if (!ret)
	...
	/*	*line = ft_strdup(newline);*/
		printf("\033[1;31mret = 0; newline : %s\033[0m\n", newline);
		printf("\033[1;32mret = 0; buf : %s\033[0m\n", buf);
		printf("\033[1;33mret = 0; line : %s\033[0m\n", *line);
		free(newline);
		/*return (0);*/
	}
	else if (ret > 0)
	{
		...
		*line = ft_strjoin(newline, tmp);
		if (ret == BUFFER_SIZE)
		{
			printf("\033[1;31mAvant-dernier tour; newline : %s\033[0m\n", newline);
			printf("\033[1;32mAvant-dernier tour; buf : %s\033[0m\n", buf);
			printf("\033[1;33mAvant-dernier tour; line : %s\033[0m\n", *line);
		}
		free(tmp);
